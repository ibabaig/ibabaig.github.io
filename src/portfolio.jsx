...
const projects = [
  ...
  { number: '04', type: 'Machine learning + uncertainty', title: 'Forecasting Grid Demand Uncertainty During Extreme Winter Weather in the Northeast with SARIMA and Bayesian Modeling Methods', description: 'Severe winter weather conditions can threaten the reliability of the power grid in various ways, such as through rising electricity demand during extreme cold, and weather-driven outages that interrupt service.', result: 'Test MAPE 11.4% overall · 9.2% during documented storm hours · 90% intervals captured 80.5% of storm-period demand', tags: ['SARIMA', 'Bayesian modeling', 'Energy forecasting'], href: 'https://github.com/rjoh10/ML2Project', cta: 'View on GitHub', reportHref: '/DS4420ProjectFinalReport.pdf', reportDetails: ['Daily electricity demand data for the Northeast Massachusetts sub-balancing authority (subba 4008) was retrieved from the U.S. Energy Information Administration (EIA) API, covering January 1, 2025 through March 30, 2026.', 'The core time series model is a Seasonal Autoregressive Integrated Moving Average (SARIMA) model, specified as SARIMA of a (1,1,1) by (1,1,1) modelled after the function below. The seasonal period was set to s=7 to reflect weekly demand cycles.', 'The Bayesian model achieved a test MAPE of 11.4% overall, improving to 9.2% during documented storm hours, with the 90% credible intervals capturing 80.5% of true storm-period demand values compared to 64.6% during non-storm hours.', 'The coefficient estimates reinforce this finding: wind events (−0.177) and snow storms (−0.124) credibly suppress demand below baseline while storm severity (+0.046) credibly increases it.'] },
];
...
function Portfolio() {
  ...
  useEffect(() => {
    const sections = ['home', 'about', 'work', 'research', 'writing'];
  ...
  return <div className="site-shell">
    <header className="topbar">
      ...
      <nav className={`main-nav ${menuOpen ? 'is-open' : ''}`}>
        {['about', 'work', 'research', 'writing'].map((item) => <a key={item} className={activeSection === item ? 'active' : ''} href={`#${item}`} onClick={() => scrollTo(item)}>{item}</a>)}
    ...
    <main>
      <section id="home" className="hero section-pad">
        <div className="hero-copy"><p className="eyebrow"><span className="eyebrow-line" /> product-minded builder · nyc / boston</p><h1 className="hero-name"><strong>Iba Baig</strong></h1><p className="hero-intro">I'm Iba (EE-Bah) — a grad student who likes sitting at the intersection of behavior and data.</p><div className="hero-actions"><a className="button button-dark" href="#work" onClick={() => scrollTo('work')}>See selected work <ArrowUpRight size={16} /></a><a className="text-link" href="mailto:ib2657@columbia.edu">Let’s talk <span>↗</span></a></div></div>
      ...
      <section id="work" className="work section-pad section-grid"><div className="section-label"><span>02</span><span>selected work</span></div><div className="work-content"><div className="section-heading-row"><h2>From first question<br /><em>to shipped thing.</em></h2><p className="section-aside">A few places I’ve practiced turning ambiguity into momentum.</p></div><div className="project-list">{projects.map((project) => <article className="project-card" key={project.number}><div className="project-number">{project.number}</div><div className="project-main"><p className="project-type">{project.type}</p><h3>{project.title}</h3><p className="project-description">{project.description}</p><p className="project-result">{project.result}</p><div className="tag-row">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>{project.reportDetails && <details className="project-details"><summary>Report details</summary><div>{project.reportDetails.map((detail) => <p key={detail}>{detail}</p>)}</div></details>}</div><div className="project-links"><a className="project-link" href={project.href} target="_blank" rel="noreferrer" aria-label={`${project.cta}: ${project.title}`}><span>{project.cta}</span><ArrowUpRight size={18} /></a>{project.reportHref && <a className="project-link project-report" href={project.reportHref} target="_blank" rel="noreferrer"><span>Read report</span><ExternalLink size={16} /></a>}</div></article>)}</div></div></section>

      <section id="research" className="research section-pad section-grid"><div className="section-label"><span>03</span><span>research</span></div><div className="writing-content"><div className="writing-intro"><h2>Questions worth<br /><em>staying with.</em></h2><p>Research projects that explore how behavior, computation, and data can help us understand the world more clearly.</p></div><div className="writing-links"><a href="https://arxiv.org/abs/2605.22962" target="_blank" rel="noreferrer"><span><small>arXiv · computer vision / human-computer interaction</small>GazeBehavior Annotation Toolkit (GBAT): AI-powered toolkit for automatic annotation of egocentric eye-tracking and video data of child-caregiver interaction</span><ExternalLink size={17} /></a><a href="/NeuroBoston%20Poster.pdf" target="_blank" rel="noreferrer"><span><small>Research poster · Northeastern University</small>NeuroBoston — research presentation</span><ExternalLink size={17} /></a></div></div></section>

      <section id="writing" className="writing section-pad section-grid"><div className="section-label"><span>04</span><span>writing</span></div><div className="writing-content"><div className="writing-intro"><h2>I like making<br /><em>the complex legible.</em></h2><p>Outside of product work, I write about science, technology, and the ideas that make the future feel a little more understandable.</p></div><div className="writing-links"><a href="https://nuscimagazine.com/predicting-the-unpredictable-the-next-frontier-in-managing-epilepsy/" target="_blank" rel="noreferrer"><span><small>NU Sci Magazine · health / neuroscience</small>Predicting the unpredictable: the next frontier in managing epilepsy</span><ExternalLink size={17} /></a><a href="https://nuscimagazine.com/3d-printing-the-brain-is-this-the-new-way-to-treat-brain-injuries/" target="_blank" rel="noreferrer"><span><small>NU Sci Magazine · technology</small>3D-printing the brain: is this the new way to treat brain injuries?</span><ExternalLink size={17} /></a></div></div></section>
